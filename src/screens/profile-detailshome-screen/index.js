          ...
          <ScrollView
            ...
            {/* {images.map((img, index) => (
              <Pressable
                ...
                key={index} style={styles.mainImage}>
                {typeof img === 'function' ? (
                  // SVG component case
                  (() => {
                    const SvgIcon = img;
            ...
            {images.map((img, index) => (
              ...
              >
                {typeof img === 'function' ? (
                  // 🔥 Instead of rendering SVG, show image2
                  <View style={[StyleSheet.absoluteFill]}>
                    <Image
                      source={profile.image2 || profile?.image} // show the PNG/JPG
                      // style={[StyleSheet.absoluteFill]}
                      style={{
                        width: SCREEN_WIDTH,
                        height: Dimensions.get('window').height,
                  ...
                  <View style={[StyleSheet.absoluteFill]}>
                    <Image
                      source={profile.image2 || profile?.image} // fallback also shows image2
                      style={{
                        position: 'absolute',
                        top: 0,
                        left: 0,
...
/> */}
        ...
        <View style={styles.contentContainer}>
          ...
          {!blurEnabled && profile?.userstatus === 'free' ? (
            <>
              <View style={{ ...styles.infoSheet, marginTop: mvs(10) }}>
                ...
                />
                {profile?.userstatus === 'free' && blurTop > 0 && (
                  <View
                    pointerEvents="none"
                    style={[
                      styles.ios26GlassWrapper,
                      { top: blurTop },
                    ]}
                  >
                    {/* iOS REAL BLUR */}
                    ...
                    {Platform.OS === 'android' && (
                      <BlurView
                        blurType="light"
                        blurAmount={1}
                        overlayColor='rgba(58, 62, 144, 0.2)'
                        style={StyleSheet.absoluteFill}
                      />
                    )}

                    {/* BLUEish GRADIENT GLASS */}
                    <LinearGradient
                      colors={[
                        'rgba(6, 6, 6, 0.25)',
              ...
              {profile?.userstatus === 'free' && blurTop > 0 && (
                ...
                >
                  ...
                  {/* INNER HIGHLIGHT */}
                  <View
                  // style={styles.glassInnerHighlight}
                  />
                </View>
              )}
          ...
          {/* {!blurEnabled && profile?.userstatus !== 'paid' && profile?.image2 && ( */}
          {!blurEnabled &&
            profile?.userstatus !== 'paid' &&
            !profile?.image2 && (
...
const styles = StyleSheet.create({
  ...
  ios26GlassWrapper: {
    ...
    left: -mvs(20),
    right: -mvs(20),
    bottom: -mvs(1000), // Cover all overflowing content
    borderBottomLeftRadius: mvs(24),
    borderBottomRightRadius: mvs(24),
    overflow: 'hidden',
    zIndex: 6,
  },
  ios26GlassWrapper2: {
    ...
    right: -mvs(20),
    // bottom:400,
    marginTop: Platform.OS === 'ios' ? mvs(250) : mvs(280),
    // top:-100,
    height: mvs(150),
    // backgroundColor:'red',
    // bottom: -mvs(10), // Cover all overflowing content
    borderBottomLeftRadius: mvs(24),
    borderBottomRightRadius: mvs(24),
    overflow: 'hidden',
    zIndex: 6,
  ...
  androidGlass: {
    backgroundColor: 'rgba(11, 10, 10, 0.45)',
  },
  ...
  infoSheet: {
    ...
    // backgroundColor: 'rgba(58, 62, 144, 0.45)',
    borderRadius: mvs(24),
    // borderWidth: 1,
    // borderColor: 'rgba(255,255,255,0.35)',
    paddingHorizontal: mvs(20),
    paddingTop: mvs(16),
    paddingBottom: mvs(16), // Added padding for bottom
  },
  infoTopRow: {
    justifyContent: 'space-between',
    marginBottom: mvs(8),
  ...
  actionButtonsRow: {
    position: 'absolute',
    bottom: mvs(40), // Adjusted to sit clearly over blur
    left: 0,
    right: 0,
    justifyContent: 'center',
    alignItems: 'center',
    columnGap: mvs(24),
    zIndex: 100, // Ensure it's on top of blur
    elevation: 10,
  },

...
export const verifyOtp = async (values: any) => {
  try {
    ...
    console.log('VERIFY OTP RAW ERROR :: ', error);

    // Axios error contains response.data
    const message =
...
export const resendOtp = async (values: any) => {
  try {
    const res = await postData(
      `${URLS.base_url}${URLS.auth.resend_otp}`,
      values
    );
    return res;
  } catch (error: any) {
    console.log('RESEND OTP RAW ERROR :: ', error);
...
export const Signup = async (values: any) => {
  try {
    console.log('values', values, 'url', `${URLS.base_url}${URLS.auth.signup}`);
    const res = await postData(`${URLS.base_url}${URLS.auth.signup}`, values);
    console.log('res of signup=>', res);
    return res; // success
} catch (error: any) {
  let msg = UTILS.returnError(error);

  // Convert array to string cleanly
  if (Array.isArray(msg)) {
    msg = msg.join(' ');
  }

  // Remove comma-separated issue
  if (typeof msg === 'string') {
    // Remove commas
    msg = msg.replace(/,/g, '');

    // Remove spaces between characters (but NOT between words)
    msg = msg.replace(/(?<=\S) (?=\S)/g, '');
  }

  Alert.alert('Error in Sign up', msg);
  return null;
}
};
...
Alert.alert("Error in Forgot Password", msg);
  }
};
...
export const onLogin = (
  ...
  return async (dispatch: AppDispatch, getState: () => RootState) => {
    try {
      ...
      return {status: false, message: UTILS.returnError(error)};
    } finally {
      setLoading(false);
    }
  };
...
export const onUpdatePassword = async (values: any) => {
  try {
    ...
    if (error?.response?.data) {
      ...
      if (data.message) message = data.message; // main message
      if (data.Error) fieldErrors = data.Error; // field-specific errors
    }

    console.log('FACTORY ERRORS :: ', error?.response?.data);
    console.log('fieldErrors =>', fieldErrors);
    console.log('API ERROR MESSAGE =>', message);

    // Throw structured error to be handled in UI
